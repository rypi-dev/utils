...
const parseExtension = (pattern: string): string | null => {
...
export const splitOverrides = (overrides: Linter.ConfigOverride[]): Linter.ConfigOverride[] => {
...
export const overrideOverrides = (overrides: Linter.ConfigOverride[]): Linter.ConfigOverride[] => {
  const result: Linter.ConfigOverride[] = []
  ...
  for (const { files, rules = {}, ...rest } of reversedOverrides) {
    ...
    if (!usedRules.has(extension)) {
      usedRules.set(extension, extensionUsedRules)
    }
    ...
    for (const [ruleName, ruleEntry] of Object.entries(rules)) {
      ...
      if (extensionUsedRules.has(ruleName)) {
        continue
      }
      if (starRules.has(ruleName)) {
        continue
      }
      overridedRules[ruleName] = ruleEntry
      extensionUsedRules.add(ruleName)
      if (extension === '*') {
        starRules.add(ruleName)
      }
    }
