...
export function createPreset<T = never>({ name, updateMeta = () => {}, compile }: CreatePresetParams<T>) {
  return (options?: T) => ({ name, options, updateMeta, compile })
}
...
export function compilePresets(presets: Preset<unknown>[], priority: string[]): Linter.Config {
